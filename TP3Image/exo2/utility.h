...
void expansionDynamique(OCTET* ImgIn, OCTET* ImgOut, int nH, int nW, int aMin, int aMax){
  for (int i = 0; i < nH; ++i){
    for (int j = 0; j < nW; ++j){
...
void expansionDynamique_Couleur(OCTET* ImgIn, OCTET* ImgOut, int nTaille3, int aMinR, int aMaxR, int aMinG, int aMaxG, int aMinB, int aMaxB){
  for (int i = 0; i < nTaille3; i += 3) {
      ImgOut[i] = (OCTET) round(255.0 / ((double)(aMaxR - aMinR)) * ((double)(-aMinR + ImgIn[i])));
      ImgOut[i+1] = (OCTET) round(255.0 / ((double)(aMaxG - aMinG)) * ((double)(-aMinG + ImgIn[i+1])));
      ImgOut[i+2] = (OCTET) round(255.0 / ((double)(aMaxB - aMinB)) * ((double)(-aMinB + ImgIn[i+2])));
  }
}
